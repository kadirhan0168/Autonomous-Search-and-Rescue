...
#
#     def navigate(self):
#         if not self.target or not self.position or not self.other_position:
#             return  # Nog niet genoeg gegevens
#
#         tx, ty = self.target
#         x, y, angle = self.position
...
#         own_distance = math.hypot(tx - x, ty - y)
#         other_distance = math.hypot(tx - ox, ty - oy)
#
#         # Alleen de dichtstbijzijnde auto mag rijden
#         if own_distance > other_distance:
#             print(f"{self.color.upper()}: Niet de dichtstbijzijnde auto, STOP.")
...
#         sys.exit(1)
#
#     # Abonneer op eigen oriëntatie, doel en andere auto
